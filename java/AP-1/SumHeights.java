 ...
 *    ending at the end index. For example, with the heights {5, 3, 6, 7, 2} and start=2, end=4 yields a
 *    sum of 1 + 5 = 6. The start end end index will both be valid indexes into the array with start <=
 *    end.
 *
 *  Tests:
...
public class SumHeights {
  public int sumHeights(int[] heights, int start, int end) {
    ...
    for (int x = start; x < end; x++) {
      sum += Math.abs(heights[x] - heights[x + 1]);
    }
    return sum;
  }
