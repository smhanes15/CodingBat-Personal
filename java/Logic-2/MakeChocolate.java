 ...
 *    kilos each). Return the number of small bars to use, assuming we always use big bars before small
...
public class MakeChocolate {
  public int makeChocolate(int small, int big, int goal) {
    ...
    int bigInGoal = (int) Math.floor(goal / 5);
    if (bigInGoal <= big) return (goal - (bigInGoal * 5));
    if (bigInGoal > big) return (goal - (big * 5));
    return -1;
  }
